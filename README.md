...
最近一次基准结果（2026-08-15，Windows 11、Python 3.12.8、本机 Docker、Mock、代码提交 `c611132`）：

```text
固定参数：20 请求、5 并发
成功：20；错误：0
吞吐量：53.62 请求/秒
平均延迟：36.86 毫秒；最大延迟：71.16 毫秒
```

报告为 `load_tests/reports/load-test-20260815T081059Z.json`。首次执行发现 Windows 系统代理会把本机请求转发并导致 HTTP 502；脚本已明确禁用系统代理继承。修复后，本次固定参数下未发现请求错误或性能瓶颈。该结果只代表一次本机 Mock 测试，不能代表高并发生产能力。
...
| 11 | 进行中 | Docker 完整启动、数据库迁移、重启数据保留和基础压力测试已验证；待最终确认 |
